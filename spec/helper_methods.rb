...
def stubbed_transactions_1_withdrawal
  transaction = double('transaction', date: Time.now.strftime('%Y-%m-%d'), debit: 800, credit: 0, balance: 200)
  transactions = [transaction]
  statement.instance_variable_set(:@transactions, transactions)
end
...
def stubbed_transactions_3
  ...
  transaction2 = double('transaction', date: Time.now.strftime('%Y-%m-%d'), debit: 0, credit: 12_000, balance: 12_200)
